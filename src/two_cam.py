...
def take_pictures():
    ...
    cam0 = Picamera2(camera_num=0) # might just work by doing 0 instead of camera_num=0 idk???
    cam1 = Picamera2(camera_num=1)

    # configure each for high-res stills
    config0 = cam0.create_still_configuration(main={"size": (4096, 4096)}, transform=Transform())
    config1 = cam1.create_still_configuration(main={"size": (4096, 4096)}, transform=Transform())
